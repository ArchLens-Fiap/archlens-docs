...
**Status**: Em andamento
**Fase Atual**: 8 - Infraestrutura K8s + Terraform (em andamento)
...
- [x] Criar `DependencyInjection.cs` (Infrastructure)
- [x] Criar Dockerfile (multi-stage build)
- [x] Configurar autenticacao no SignalR (JWT) - JWT Bearer + query-string token
- [ ] Testes do Hub
...
### ADR Review (Fase 8)
- [x] Revisar ADRs existentes
- [x] Criar ADR para: KIND vs Minikube vs k3d (cluster local)
- [x] Criar ADR para: Kustomize vs Helm (gerenciamento de manifests)
...
- [x] Criar estrutura `k8s/base/`, `k8s/overlays/local/`, `k8s/overlays/dev/` (archlens-infra-k8s)
- [x] Criar deployments, services, HPA para cada servico em `k8s/base/`
  - [x] Configurar probes (startup, readiness, liveness) em cada deployment
  - [x] Configurar security context (runAsNonRoot, drop capabilities)
  - [x] Configurar resource requests/limits
  - [x] Configurar HPA (CPU 70%, Memory 80%, min 2, max 10)
  - [x] Criar ConfigMaps e Secrets
  - [x] Criar Ingress NGINX
  - [x] Criar overlay `k8s/overlays/local/` (KIND-specific)
  - [x] Criar overlay `k8s/overlays/dev/` (para futuro cloud)

### 8.3 Terraform
- [x] Criar estrutura `terraform/modules/` com kind-cluster, ingress-nginx, kustomize (archlens-infra-k8s)
- [x] Implementar modulo `kind-cluster` (provisiona cluster KIND)
  - [x] Criar modulo `ingress-nginx` (instala NGINX ingress no KIND)
  - [x] Criar modulo `kustomize` (aplica manifests)
  - [x] Criar `main.tf` + `variables.tf` + `outputs.tf`
...
- [ ] Criar ADR-010: Estrategia de Conformidade LGPD

### 9.1 Seguranca
- [x] Validacao de entrada em todos os endpoints (FluentValidation)
- [x] Verificar magic bytes no upload (nao so extensao)
- [x] Sanitizacao de nomes de arquivo
- [x] Configurar HTTPS no Ingress
- [x] Configurar OWASP headers (X-Content-Type-Options, X-Frame-Options, etc.)
- [x] Revisar secrets (nenhum hardcoded, tudo via env vars)
- [x] Scan Trivy em todas as imagens finais
- [x] Documentar riscos e limitacoes de seguranca
...
- [x] Documentar quais dados pessoais o sistema coleta (email, nome, IP, uploads)
- [x] Classificar dados por sensibilidade (dados cadastrais vs dados de uso)
- [x] Documentar finalidade de cada dado coletado (Art. 7 LGPD - base legal)
- [x] Documentar tempo de retencao de cada tipo de dado

**Direitos do Titular (Art. 17-22 LGPD)**
- [x] Endpoint DELETE /api/auth/me - exclusao de conta e dados (direito ao esquecimento)
- [x] Endpoint GET /api/auth/me/data - exportacao dos dados do usuario (portabilidade)
- [x] Cascata de exclusao: deletar usuario -> deletar uploads -> deletar reports -> deletar notifications
- [x] Anonimizacao de dados em logs (nao logar email/nome em plaintext)

**Consentimento e Transparencia**
- [x] Tela de consentimento no registro (checkbox LGPD com texto claro)
- [x] Pagina /privacy-policy no frontend (politica de privacidade)
- [x] Pagina /terms no frontend (termos de uso)
- [x] Banner de cookies (se aplicavel)

**IA e LGPD (Aula 4 - IA na LGPD)**
- [x] Documentar que dados do usuario sao enviados para IAs externas (OpenAI, Google, Anthropic)
- [x] Informar usuario que diagramas serao processados por IA (consentimento explicito no upload)
- [x] Garantir que nenhum dado pessoal do usuario e enviado aos providers de IA (apenas o diagrama)
- [x] Documentar que nao ha decisao automatizada com efeito legal (Art. 20 LGPD)
- [ ] Transparencia: mostrar quais providers analisaram cada diagrama (ja temos no report)

**Medidas Tecnicas de Protecao**
- [x] Criptografia em transito (HTTPS/TLS entre todos os servicos)
- [ ] Criptografia em repouso (PostgreSQL encryption, MinIO SSE)
- [x] Audit log de acesso a dados pessoais (quem acessou o que e quando)
- [x] Data retention policy: deletar uploads antigos apos X dias (configuravel)
- [x] Minimizacao de dados: nao coletar mais do que o necessario

**Documentacao LGPD (para o README e video)**
- [x] Secao LGPD no README com medidas adotadas
- [x] Diagrama de fluxo de dados pessoais (data flow diagram)
- [x] Registro de atividades de tratamento (ROPA simplificado)
...
- [x] Configurar OpenTelemetry em `archlens-auth-service` (ASP.NET + HTTP + Runtime + OTLP exporter)
- [x] Configurar OpenTelemetry em `archlens-upload-service` (ASP.NET + HTTP + Runtime + OTLP exporter)
- [x] Configurar OpenTelemetry em `archlens-orchestrator-service` (ASP.NET + HTTP + Runtime + OTLP exporter)
- [x] Configurar OpenTelemetry em `archlens-notification-service` (ASP.NET + HTTP + Runtime + OTLP exporter)
- [x] Configurar OpenTelemetry em `archlens-report-service` (ASP.NET + HTTP + Runtime + OTLP exporter)
- [x] Configurar OpenTelemetry em `archlens-gateway` (ASP.NET + HTTP + Runtime + OTLP exporter)
- [ ] Verificar trace distribuido completo no Jaeger (.NET -> RabbitMQ -> Python -> .NET)
...
- [x] Configurar Rate Limiting em todos os servicos (FixedWindowRateLimiter + auth-strict)
- [x] Configurar Swagger com JWT Bearer (SecurityDefinition Bearer)
...
**Antes da Fase 8 (K8s + Terraform): CONCLUIDO**
...
- [x] Criar estrutura `k8s/base/`, `k8s/overlays/local/`, `k8s/overlays/dev/` no archlens-infra-k8s
- [x] Criar estrutura `terraform/modules/` (kind-cluster, ingress-nginx, kustomize) no archlens-infra-k8s
- [x] .gitignore do archlens-infra-k8s atualizado (89 linhas - .NET, Python, Node, Terraform, IDE, env)
...
| 11/03/2026 | Fase 8 (prep) | archlens-infra-k8s: estrutura k8s/base, k8s/overlays/local, k8s/overlays/dev, terraform/modules/(kind-cluster,ingress-nginx,kustomize) criada e pushada. .gitignore atualizado (89 linhas). Todos os 12 repos clonados. Todos os builds .NET OK (0 erros). |
| 11/03/2026 | Fase 8 (k8s+terraform) | Manifestos K8s completos: namespace, 7 secrets, configmap, 5 infra (postgres+init-script, mongodb, rabbitmq, redis, minio+bucket-init), 8 app deployments+services (auth, upload, orchestrator, notification, report, ai-processing, gateway, frontend), 8 HPAs (CPU 70%, Mem 80%, min 2, max 10), Ingress NGINX. Kustomize: base/kustomization.yaml + overlays/local (NodePort+StorageClass) + overlays/dev. Terraform: 3 modulos (kind-cluster, ingress-nginx, kustomize) + root main/variables/outputs. Security: runAsNonRoot, drop ALL caps, readOnlyRootFilesystem em todos apps .NET. |
| | | |
